...
def estimate_landing(speed_kph, dist_km):
    ...
    eta_mins = (dist_km / (speed_kph or 850)) * 60
...
# --- 6. RENDERER (FIXED KEYWORD ARG ERROR) ---
def render_flight_status(data, force_fnum=None): # Fixed naming to match calling code
    flights = data.get("response", {}).get("flights", []) if data else []
    
    live = None
    if force_fnum:
        live = get_airlabs_data(force_fnum, mode="live")
        if not live:
            live = get_airlabs_data(force_fnum, mode="schedules")
            if live: log_debug("AirLabs Schedule Fallback", live)

    if not flights and not live:
        st.error(f"🛑 Flight SQ{force_fnum} not found.")
        return

    # Use Schedule data if SIA is empty
    if not flights and live:
        flights = [{"legs": [{"flightNumber": force_fnum, "flightStatus": live.get("status", "Scheduled")}]}]
    ...
    for f in flights:
        for leg in f.get("legs", []):
            ...
            # Metadata Merge
            ac_type = live.get("model") if live else leg.get("aircraft", {}).get("displayName", "SIA Jet")
            ...
            # ACTUAL CARD RENDERING (RESTORED)
            html = (
                ...
                f"</div>"
                f"<div class='telemetry-footer'>📡 Telemetry: Spd: {live.get('speed', '0') if live else '---'}km/h | Alt: {live.get('alt', '0') if live else '---'}ft | Landing: {estimate_landing(live.get('speed', 0), 500) if live else '---'}</div>"
                f"</div>"
            )
    ...
    try:
        ...
        for s in (states or []):
            if str(s[1]).strip().startswith("SIA"):
                ...
                fleet.append({"Callsign": s[1].strip(), "Reg": s[0].upper(), "Lat": s[6], "Lon": s[5], "Alt (ft)": int(alt * 3.28), "Mach": get_mach(gs, alt)})
        
        c1, c2 = st.columns([3, 1])
        with c1:
            m = folium.Map(location=[1.35, 103.98], zoom_start=3, tiles='CartoDB dark_matter')
            for ac in fleet: folium.Marker([ac['Lat'], ac['Lon']], popup=f"{ac['Callsign']}").add_to(m)
