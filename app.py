...
st.markdown(f"""
    ...
    .stButton>button {{ background-color: {SIA_GOLD} !important; color: {SIA_NAVY} !important; font-weight: bold; }}
    </style>
...
# --- 3. LOGIC FUNCTIONS ---
def get_sia_headers(api_type="status"):
    key = st.secrets["SIA_STATUS_KEY"]
    secret = st.secrets.get("SIA_STATUS_SECRET", "") # Handle empty secret
...
def get_live_sq_fleet():
    """Fetches real-time GPS coordinates for all SIA planes from OpenSky."""
    url = "https://opensky-network.org/api/states/all"
    auth = (st.secrets["OPENSKY_CLIENT_ID"], st.secrets["OPENSKY_CLIENT_SECRET"])
    try:
        r = requests.get(url, auth=auth, timeout=10)
        states = r.json().get("states", [])
        # Filter for 'SIA' (Singapore Airlines ICAO code)
        return [s for s in states if s[1].strip().startswith("SIA")]
    except: return []
...
with tab1:
    ...
    if st.button("Get Official Status"):
        ...
        try:
            ...
            st.markdown(f"""
                <div class="flight-card">
                    ...
                    <p>{data['origin']['airportCode']} ({data['origin']['cityName']}) ➔ {data['destination']['airportCode']} ({data['destination']['cityName']})</p>
                    <p><b>Departure:</b> {data['scheduledDepartureTime']} | <b>Arrival:</b> {data['scheduledArrivalTime']}</p>
                </div>
            """, unsafe_allow_html=True)
        ...
        except: st.error("Flight data unavailable. Note: Sandbox typically only supports SQ638.")

with tab2:
    st.subheader("Global Live SIA Positions (OpenSky)")
    if st.button("Refresh Fleet Radar"):
        ...
        for plane in fleet:
            # s[6]=lat, s[5]=lon, s[1]=callsign, s[7]=alt
            folium.Marker(
                [plane[6], plane[5]], 
                popup=f"Callsign: {plane[1].strip()}",
                icon=folium.Icon(color='orange', icon='plane')
            ).add_to(m)
            
        st_folium(m, width="100%", height=500)
        st.write(f"Currently tracking **{len(fleet)}** SIA aircraft worldwide.")
